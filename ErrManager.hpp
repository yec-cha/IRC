...
public:
	...
	static void send_482(int socket, const std::string &nickName, const std::string &channelName)
	{
		const std::string response = "482 " + nickName + " " + channelName + " :You're not channel operator\n";
		sendClient(socket, response);
	};
